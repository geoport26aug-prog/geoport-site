...
.lens{display:none;position:absolute;width:170px;height:170px;border-radius:50%;border:2px solid rgba(28,95,176,.7);box-shadow:0 6px 20px rgba(15,47,92,.28);background-color:#fff;background-repeat:no-repeat;pointer-events:none;z-index:5}
@media (hover:none){.lens{display:none!important}}
.zhint{position:absolute;left:10px;bottom:10px;background:rgba(15,47,92,.72);color:#fff;font-size:10px;padding:3px 8px;border-radius:5px;pointer-events:none;letter-spacing:.5px}
/* 全画面拡大（PC・スマホ共通） */
...
def render(row, slug, g=None, pos=0):
    ...
    if img_url:
        ...
        photo_html = (f'<div><div class="photo zoomable" id="photobox">'
                      ...
                      f'onclick="openLB()"><span class="zhint">押すと拡大</span></div>'
                      f'{thumbs}</div>')
    else:
...
/* 虫眼鏡：マウスのある端末だけ。倍率は写真の実サイズ÷表示サイズ（約2.3倍） */
(function(){{
  if(!window.matchMedia||!matchMedia("(hover:hover)").matches)return;
  var box=$("photobox"),img=$("mainshot");if(!box||!img)return;
  var L=170,lens=document.createElement("div");lens.className="lens";box.appendChild(lens);
  box.addEventListener("mousemove",function(ev){{
    if(!img.naturalWidth){{lens.style.display="none";return;}}
    ...
    lens.style.left=(ev.clientX-br.left-L/2)+"px";
    lens.style.top=(ev.clientY-br.top-L/2)+"px";
    lens.style.backgroundImage='url("'+img.src+'")';
    lens.style.backgroundSize=img.naturalWidth+"px "+img.naturalHeight+"px";
    lens.style.backgroundPosition=(-(x*zx-L/2))+"px "+(-(y*zy-L/2))+"px";
  }});
  box.addEventListener("mouseleave",function(){{lens.style.display="none";}});
}})();
