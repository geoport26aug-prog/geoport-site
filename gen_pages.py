...
def render(row, slug, g=None, pos=0, gone=False):
    ...
    if gone:
        ...
        price_row = ('<tr><th>価格</th><td><button class="cta" type="button" '
                     # ★2026-09-10 S決定：「この製品の見積を確認する」→「お見積り・お問い合わせ」。
                     #   ・「確認する」だと値段が既に載っているように読める（実際に開くのは依頼フォーム）。
                     #   ・「お問い合わせ」を足す理由＝値段以外（在庫・仕様）を聞きたい客も押せるように。
                     #     押した先のフォームの見出しは「見積・在庫確認」で、備考欄に何でも書ける。
                     #   ・送り仮名はサイト・見積メール・見積書PDFの「お見積り」に揃える（「お見積もり」は使わない）。
                     #   ・幅は実測で 194px（欄389px／スマホの欄220px にも1行で収まる）。
                     #     「お見積り」だけだと96pxで、主役のボタンとして小さすぎた。
                     'onclick="openQuote()">お見積り・お問い合わせ</button></td></tr>')
...
<div class="hp" aria-hidden="true"><label for="f-web">Website</label><input id="f-web" type="text" name="website" tabindex="-1" autocomplete="off"></div>
